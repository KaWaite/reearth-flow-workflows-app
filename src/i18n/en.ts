...
export const en: Translations = {
  ...
  workflows: {
    csvQuality: {
      ...
      fields: {
        csvPath: { label: 'CSV Path', hint: 'URL or path to the input CSV file.' },
        keyColumn: { label: 'Key Column', hint: 'Rows where this column is null or empty are rejected.' },
        outputPrefix: { label: 'Output Prefix', hint: 'Output files: <code>{prefix}_clean.csv</code> and <code>{prefix}_rejected.csv</code>' },
      },
      alert: 'Set <code>FLOW_URL_CSV_QUALITY</code> (variable) and <code>FLOW_API_KEY</code> (secret) in your GitHub repository settings, then redeploy. For local dev, add them to <code>.env.local</code>.',
    },
    geoJson: {
      ...
      fields: {
        geojsonPath: { label: 'GeoJSON Path', hint: 'URL to a GeoJSON file with polygon features in a metric CRS (e.g. EPSG:2154, 3857). The workflow reprojects to WGS84 (EPSG:4326) for the Cesium output.' },
        categoryColumn: { label: 'Category Column', hint: 'Features are grouped by this attribute when counting. Must exist on every feature.' },
        outputName: { label: 'Output Name', hint: 'Output files: <code>{name}.geojson</code> and <code>{name}_stats.json</code>' },
      },
      alert: 'Set <code>FLOW_URL_GEOJSON</code> (variable) and <code>FLOW_API_KEY</code> (secret) in your GitHub repository settings, then redeploy. For local dev, add them to <code>.env.local</code>.',
    },
    csvMerge: {
      ...
      fields: {
        ...
        dedupKey: { label: 'Dedup Key Column', hint: 'Rows with the same value in this column are considered duplicates; only the first occurrence is kept.' },
        outputPrefix: { label: 'Output Prefix', hint: 'Output file: <code>{prefix}.csv</code>' },
      },
      alert: 'Set <code>FLOW_URL_CSV_MERGE</code> (variable) and <code>FLOW_API_KEY</code> (secret) in your GitHub repository settings, then redeploy. For local dev, add them to <code>.env.local</code>.',
    },
    colSelect: {
      ...
      fields: {
        csvPath: { label: 'CSV Path', hint: 'URL or path to the input CSV file.' },
        columnsToRemove: { label: 'Columns to Remove', hint: 'Comma-separated column names to drop. All other columns are kept as-is.' },
        outputName: { label: 'Output Name', hint: 'Output file: <code>{name}.csv</code>' },
      },
      alert: 'Set <code>FLOW_URL_COL_SELECT</code> (variable) and <code>FLOW_API_KEY</code> (secret) in your GitHub repository settings, then redeploy. For local dev, add them to <code>.env.local</code>.',
    },
    csvSplit: {
      ...
      fields: {
        csvPath: { label: 'CSV Path', hint: 'URL or path to the input CSV file.' },
        categoryColumn: { label: 'Category Column', hint: 'Column whose values determine which output file each row goes to. The set of categories is defined in the workflow — rows not matching any category go to a catch-all file.' },
        outputPrefix: { label: 'Output Prefix', hint: 'Output files: <code>{prefix}_&lt;category&gt;.csv</code>' },
      },
      alert: 'Set <code>FLOW_URL_CSV_SPLIT</code> (variable) and <code>FLOW_API_KEY</code> (secret) in your GitHub repository settings, then redeploy. For local dev, add them to <code>.env.local</code>.',
    },
    csvToJson: {
      title: 'CSV to JSON',
      ...
      fields: {
        csvPath: { label: 'CSV Path', hint: 'URL or path to the input CSV file.' },
        outputName: { label: 'Output Name', hint: 'Output file: <code>{name}.json</code>' },
      },
      alert: 'Set <code>FLOW_URL_CSV_JSON</code> (variable) and <code>FLOW_API_KEY</code> (secret) in your GitHub repository settings, then redeploy. For local dev, add them to <code>.env.local</code>.',
    },
    spatialFilter: {
      ...
      fields: {
        geojsonPath: { label: 'GeoJSON Path', hint: 'URL to a GeoJSON file with polygon features in a metric CRS (e.g. EPSG:2154, 3857).' },
        minArea: { label: 'Minimum Area (m²)', hint: 'Features with area below this value are discarded.' },
        outputName: { label: 'Output Name', hint: 'Output file: <code>{name}.geojson</code>' },
      },
      minAreaThat: "That's",
      minAreaThatSuffix: '.',
      alert: 'Set <code>FLOW_URL_SPATIAL_FILTER</code> (variable) and <code>FLOW_API_KEY</code> (secret) in your GitHub repository settings, then redeploy. For local dev, add them to <code>.env.local</code>.',
    },
  },
  setupGuide: {
